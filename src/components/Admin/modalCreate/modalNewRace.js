...
export default function ModalNewRace({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim()) {
      setStatus("error");
      setErrorMessage("Por favor, insira um nome para a raça");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.detail ||
            data.message ||
            "Erro ao criar raça. Verifique os dados e tente novamente."
        );
      }

      if (typeof onRaceCreated === "function") {
        onRaceCreated(data);
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="Ex: Meio Demônio, Anão, Elfo"
              required
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder="Descreva as características desta raça"
            />
          </div>
